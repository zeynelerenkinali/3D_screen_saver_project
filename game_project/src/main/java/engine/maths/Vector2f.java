...
public class Vector2f {
    ...
    public static float length(Vector2f vector){
        return (float) Math.sqrt(vector.getX() * vector.getX() + vector.getY() * vector.getY()); // len of the vector based on the 0,0,0 origin
    }

    public static Vector2f normalize(Vector2f vector){ // can be used when the situation is we care only direction not the magnitude. 
        float len = Vector2f.length(vector);
        return Vector2f.divide(vector, new Vector2f(len, len));
    }

    public static float dot(Vector2f vector1, Vector2f vector2){ // useful for light
